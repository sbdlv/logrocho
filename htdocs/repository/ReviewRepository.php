...
class ReviewRepository implements IDAO
{
    ...
    {
        $stmt = getConexion()->prepare("SELECT r.*, SUM(CASE WHEN rul.isLike = 1 THEN 1 ELSE 0 END) as likes, SUM(CASE WHEN rul.isLike = 0 THEN 1 ELSE 0 END) as dislikes FROM `review` r  JOIN review_user_likes rul ON r.id = rul.review_id WHERE rul.user_id = ? AND rul.isLike = 1 GROUP BY r.id");
        $stmt->execute([$id]);
    ...
    {
        $stmt = getConexion()->prepare("SELECT r.*, SUM(CASE WHEN rul.isLike = 1 THEN 1 ELSE 0 END) as likes, SUM(CASE WHEN rul.isLike = 0 THEN 1 ELSE 0 END) as dislikes FROM `review` r  JOIN review_user_likes rul ON r.id = rul.review_id WHERE rul.user_id = ? AND rul.isLike = 0 GROUP BY r.id");
        $stmt->execute([$id]);
